...
function textParser(req, res, next) {
  ...
  if (type === undefined || "text/plain" !== type.split(";")[0]) {
    return next();
  }
  ...
  csv()
    ...
    .on("data", function(data) {
      ...
      if (potentialTrips === undefined) {
        console.log(
          "Could not find trips corresponding to an AVL work piece ID: " +
            workId
        );
        workMissCount += 1;
      ...
      if (avlTrip === null) {
        console.log(
          util.format(
            "Could not find a trip for work piece: %s and timestamp: %s",
            workId,
            timestamp
          )
        );
    ...
    .on("end", function(count) {
      ...
      if (FeedMessage) {
        console.log("Creating GTFS-relatime from", JSON.stringify(feedMessage));
        var message = FeedMessage.fromObject(feedMessage); // or use .fromObject if conversion is necessary
        serializedFeed = FeedMessage.encode(message).finish();
      } else {
        console.error(
          "Error: no FeedMessage -- was the protobuf definition loaded OK?"
        );
      }

      console.log(
        "Created GTFS-Realtime data from " + count + " rows of AVL data."
      );
      console.log("Could not resolve " + tripMissCount + " AVL trip IDs:");
      console.log(tripMissList);
      console.log(
        "Could not resolve " + workMissCount + " AVL work piece IDs."
      );
    });
}
